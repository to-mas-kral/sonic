...
int main() {
    ...
    u32 blocks = (num_samples + threads_per_block - 1U) / threads_per_block;

    fmt::println("Rendering a {}x{} image at {} samples.", image_x, image_y, num_samples);
    fmt::println("{} samples split into {} blocks with {} samples each.", num_samples, blocks,
                 threads_per_block);

    auto meshes = init_meshes();

    RenderContext *rc = nullptr;
    cudaMallocManaged((void **)&rc, sizeof(RenderContext));
    *rc = RenderContext(std::move(meshes), image_x, image_y, num_samples);

    for (u32 x = 0; x < image_x; x++) {
        for (u32 y = 0; y < image_y; y++) {
            render_megakernel<<<blocks, threads_per_block>>>(rc, x, y, threads_per_block);
        }
    }

    cudaFree(rc);

    /*int num_pixels = image_x * image_y;
    ...
    create_world<<<1, 1>>>(d_list, d_world, d_camera, image_x, image_y, d_rand_state2);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
    ...
    start = clock();
    // Render our buffer
    dim3 blocks_old(image_x / threads_x + 1, image_y / threads_y + 1);
    dim3 threads(threads_x, threads_y);
    render_init<<<blocks_old, threads>>>(image_x, image_y, d_rand_state);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
    render<<<blocks_old, threads>>>(fb, image_x, image_y, num_samples, d_camera, d_world,
    d_rand_state); checkCudaErrors(cudaGetLastError()); checkCudaErrors(cudaDeviceSynchronize());
    stop = clock();
