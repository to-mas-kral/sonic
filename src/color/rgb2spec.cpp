...
RGB2Spec::RGB2Spec(const std::filesystem::path &path) {
...
f32
RGB2Spec::eval(const tuple3 &coeff, f32 lambda) {
...
i32
RGB2Spec::rgb2spec_find_interval(const f32 *values, f32 x) const {
    i32 left = 0;
...
f32
RGB2Spec::rgb2spec_fma(f32 a, f32 b, f32 c) {
    return a * b + c;
}
