...
RGB2SpecSTRUCT *rgb2spec_load(const char *filename) {
    ...
    RGB2SpecSTRUCT *m = (RGB2SpecSTRUCT *) malloc(sizeof(RGB2SpecSTRUCT));
...
void rgb2spec_free(RGB2SpecSTRUCT *model) {
    free(model->scale);
    free(model->data);
    free(model);
}
...
void rgb2spec_fetch(RGB2SpecSTRUCT *model, float rgb_[3], float out[RGB2SPEC_N_COEFFS]) {
